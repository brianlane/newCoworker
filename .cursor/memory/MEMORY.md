...
- [HomeLight portal traps](project_homelight_portal_traps.md) - email-first login (probe = false login_failed); hapi cookie; templated click_text; click_text can miss a header that is already in the later screenshot (run 39f53cb7, `CLICK_TEXT_APPEAR_MS` 5s to 15s, nav is the href selector); THREE price channels, IVR can disagree with the written pair; text claims reveal the contact card; `claim_state` can read our own Claimed By as a rival
