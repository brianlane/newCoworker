...
- [A ChatGPT app IS an MCP server](project_chatgpt_app_is_an_mcp_server.md) - v1.0.0 PUBLISHED Sep 23 2026 at chatgpt.com/plugins; directory install is the customer path, pasted URL is the developer-mode fallback; sandbox texts Brian BY DESIGN
...
- [MCP outreach queue tools](project_mcp_outreach_drafts.md) - paragraphs only, footer in code; upsert re-pitches only discovered/drafted rows; one write straight to drafted; findings empty so Write it again refuses; auto mode SENDS; five guarded places per new MCP tool; first email asks, no booking link by default since Sep 2026 (include_booking_link per draft, stored); pitch_body is frozen at draft time so flipping the setting needs reassemble-outreach-drafts.ts
- [HQ postal-address waiver](project_hq_postal_address_waiver.md) - created Standard 2026-07-16, dossier said enterprise with no write, set to enterprise 2026-09-22 without moving the kvm1 box; postal waiver is the plan plus HQ_BUSINESS_ID; do not widen to other Standard tenants
...
- [HomeLight portal traps](project_homelight_portal_traps.md) - email-first login (probe = false login_failed); hapi cookie; templated click_text; click_text can miss a header that is already in the later screenshot (run 39f53cb7, `CLICK_TEXT_APPEAR_MS` 5s to 15s, nav is the href selector); THREE price channels, IVR can disagree with the written pair; text claims reveal the contact card; `claim_state` can read our own Claimed By as a rival; the post-click already-claimed overlay is also ours (Debra M. run aaeb08fb: do not MMS it, only the contact card); an unclicked "Call me to claim referral" button is NOT CONFIRMED even when the model says "Call me again" (Ron G. run e7c5e9b3, `forceWhenText`)
- [HomeLight text-then-link + lost-offer](project_homelight_text_then_link_and_lost_offer.md) - persist inbound before eval; URL-dedupe the withdrawal SMS; never offer a lost claim; wait then offer only if the call connected; Sonia R. 2026-09-13
...
- [Hostinger purchase reply shape](project_hostinger_purchase_response_shape.md) - { order, virtual_machine } SINGULAR; we never parsed it, so "fail-but-charge" was partly ours; post-charge failures covered too; a NAMED vm skips the age heuristics; --adopt-vm recipe
...
- [Plan-change cutover assignment](project_plan_change_cutover_assignment.md) - do not release or replace a box with prepaid Hostinger time; write businesses.tier both directions; migrate only at/past expires_at; KIN 2026-09-18 was pooled-with-no-replacement
...
- [Coworker channel architecture](project_coworker_channel_architecture.md) - shared pipeline + the 12 non-enforced widening sites, all in-product; the public site is the blind spot
...
- [Telnyx voice rate deck zones](project_telnyx_voice_rate_deck_zones.md) - zoned per Terminating LRN, not always dialed NPA; US Zone 5 is 14x; weighted voice units (short-leg cap 20, full raw past 60 billable seconds) meter the tenant pool like SMS; Aug 28 dialed-NPA history read 100% Zone 1; Sep 2026 MDR showed Payson LRN Zone 5; flat 0.9c/min holds; #1809 closed
...
- [Hostinger list flake is not ACTION REQUIRED](project_hostinger_list_flake_is_not_action_required.md) - catalog/billing-list timeout retries once, then warn-until-repeat in 48h; watchdog must not page CRASHED; Sep 11 2026 mail was this flake, no box touched
...
- [Voice hangup named the tool](project_voice_hangup_narration.md) - HQ f76c30c0 said "Calling the end call tool now"; the goodbye turn's only tool call is end_call; other tools still get a plain sentence; ack is stay_silent; keep BLOCKING
...
- [GSC /es canonical must be self](project_gsc_es_canonical_self.md) - marketing HTML canonical follows x-pathname, not the locale cookie; legal /es twins stay English-canonical and off the sitemap; distinct from apex/www 308s and the Aug 403s
- [Homepage URL has no trailing slash](project_homepage_url_no_slash.md) - canonical, sitemap loc, hreflang, JSON-LD, llms Home all use `siteUrl("/")` (SITE_URL, no slash). Next emits URL.origin for `/`. Sitemap must not concatenate SITE_URL + "/"
