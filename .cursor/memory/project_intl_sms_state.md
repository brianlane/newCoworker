...
**Sep 5 2026, the honesty half shipped (Brian-approved product change).** A LEAD-facing skip now (a) emails the lead the same message when an address exists: the step's after-hours `emailFallbackVar` first, else `contacts.email` for the number (alias-aware), through `deliverFlowEmail` (tenant AI mailbox, `email_log`, per run+step Resend idempotency), never throwing (a Resend failure is `email_fallback: "email_failed"` on the step, not a dead-lettered run); and (b) records the skip in `scope.vars.__untextable_sms` (`_shared/ai_flows/untextable_sms.ts`, JSON string like the other engine markers). The planner appends the note to every later `notify_owner` / `notify_lead_owner` message and the worker appends it to teammate texts, so "I sent them the greeting" and "no reply to 3 follow-ups" end with the truth (number, country, no text went out, emailed-or-not). When no owner-alert step is about to fire (none ahead, or its `when` fails on current vars, or its branch is undecided), the worker sends ONE standalone alert through `notifyOwnerStep` at the skipped step's index (`ai_flow_sms_international_owner_alerted`); a flow alert carrying the note marks the run `told`, so the two never stack. Roster skips are untouched (no email, no record). The after-hours branch and this skip share the `_after_hours_emailed_<index>` marker so an overnight nudge is not emailed twice. Test seam: `RESEND_API_BASE` (`_shared/resend_api_base.ts`, honored by the worker and the notifications function) points the itest env at the fake app's `/emails`.

Deliberately NOT done: the bad-phone intake arm (lead_phone = "none") still does not fire for a real international number (blanking would strip the number from the contact and alerts); an email reply does not resume a phone-keyed `wait_for_reply`, so the ladder runs its full cadence by email; plain `send_email` teammate touches (VFM's Liz emails) are not annotated because a literal-address send_email cannot be told apart from a lead email. KYP-specific: the standalone alert rides the same notify_owner ladder as every KYP owner alert, i.e. SMS to the unread `+1514` forward number, unless that number is absent or non-NANP. Related: [[vfm-second-brand-kyp]], [[telnyx-number-level-sms-block]].
