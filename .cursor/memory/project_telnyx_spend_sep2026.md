...
RCS agent `new_coworker_jut3q1af_agent` is still LIVE in the testing phase (`NON_CONVERSATIONAL`, one tester device). Production RCS fees ($600 + $100/mo) were deferred Jul 18 and are not in this bill. Truly Insurance's DID `+15198006401` was still on the account through the Sep 7 grace window on purpose (see the grace-hold note below). August usage on that number was ~$0. The grace wipe completed 2026-09-08T00:15:05.524Z and the number is released.
...
Truly's DID rented $1.10 through the 30-day grace hold; that was not a forgotten leak. Re-checked live 2026-09-02 (still in grace): Telnyx listed `+15198006401` as `active` (`release_in_progress=false`, connection "Truly Insurance"); `business_telnyx_settings` and `telnyx_voice_routes` still pointed at it. Subscription `ef8f9eba-0a86-4037-8f51-024dc10be5e5` was `canceled` (`canceled_at` 2026-08-08, `cancel_reason=user_period_end`), `grace_ends_at=2026-09-07T22:27:42.88Z`, `wiped_at` then null. Lifecycle kept the number through grace so a reactivation would have gotten the same line (`release_did` fires on the terminal wipe or admin force-cancel). Sep 1 MRC billed it with the other five numbers. Grace wipe completed 2026-09-08T00:15:05.524Z: `businesses.status=wiped`, DID released, `telnyx_voice_routes` empty, `telnyx_sms_from_e164` null. Fleet DID count should now be five (Truly gone). HQ has $0 membership revenue and still costs hosting. Neither is an SMS-cap problem. See `docs/tenants/truly-insurance.md`.
