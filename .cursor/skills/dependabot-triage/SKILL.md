...
| A lockfile tree is missed entirely | This repo has several: root, `vps/chat-worker`, `vps/voice-bridge`, `vps/data-api`, `vps/aiflow-render`, `cloudflare/email-worker`, `zapier` | Dependency Audit CI covers every tree (PR #743); `zapier/` is also in `.github/dependabot.yml` so the lockfile refreshes weekly. Fix the tree the alert names |
| An override is stuck on a vulnerable lockfile copy after a patched release ships | Dependabot does not bump `overrides`. A `>=` floor does not regenerate the lockfile by itself. | Add a row to `.github/unpatched-release-watch.json` when you pin because **no** patched release exists. The Monday workflow opens one issue when npm has `minRelease` and the lockfile is still behind. Bump the override + lockfile, then **delete the row**. |
...
| A vitest / `@vitest/*` PR fails `npm ci` with ERESOLVE (peer vitest@"x.y.z") | `@vitest/coverage-v8` and `@vitest/ui` peer an exact `vitest` version. Separate PRs mix majors (Dependabot #1819, #1820, #1821). | Close the split PRs. Bump all three together. The root `vitest` group in `.github/dependabot.yml` must stay; never drop it back into `minor-and-patch` (that group drops majors). `tests/vitest-peer-lockstep.test.ts` is the guard. |
