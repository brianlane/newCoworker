...
| `backfill-nango-account-identity.ts` | **Data backfill.** Stamps `provider_account_email` / `provider_account_display_name` onto `workspace_oauth_connections.metadata` by asking each provider (Gmail profile, Graph `/me`, …) whose account the token is for — pre-fix Connect-UI rows were labeled with the dashboard login instead. Idempotent — skips rows that already have the key. Dry-run by default; `--apply` to write; `--business <uuid>` to scope. |
