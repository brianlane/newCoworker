...
| `rowboat-logs.ts` | Tail a tenant's Rowboat container logs filtered by pattern (`tool\|webhook\|send_sms` by default): `tsx debug/rowboat-logs.ts [businessId] [pattern] [--since=15m] [--tail=40]`. The Rowboat-side complement to `logs.ts`. Read-only. |
