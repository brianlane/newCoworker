...
| `image-size: ^2.0.4` | Sep 25, 2026 (Dependabot alert #67) | GHSA-5p2g-fcmc-qvqq / CVE-2025-71329 (high, JXL/HEIF infinite loop) and GHSA-w3rx-r6r6-pgpr (high, ICNS infinite loop). Both are fixed in 2.0.3; 2.0.4 is the current release. `pptxgenjs` 4.0.1 depends on `^1.2.1`, which cannot resolve to 2.x, so a direct dependency does not move the nested copy. | **Live.** The PowerPoint exporter (`src/lib/pptx/from-markdown.ts`) builds text-only decks, and pptxgenjs 4.0.1 never loads `image-size`. The pin stays so a later `addImage` cannot reinstall 1.2.1. |
...
| `js-yaml: ^4.3.2` | Sep 2026 (Dependabot alert #65) | GHSA-2883-xcg3-v3hh / CVE-2026-84375 (high): `maxTotalMergeKeys` does not count empty merge sources, so a small YAML document can burn CPU. Reached through `eslint` → `@eslint/eslintrc` 3.3.6 (`^4.3.0`, resolved 4.3.1). 4.3.2 is the patched 4.x line; do not raise this to 5.x (eslint stays on 4). `@eslint/eslintrc` 3.3.7 already requires `^4.3.2`; the override keeps nested copies on the patch if a later eslint bump re-pulls 3.3.6. | **Live.** Dev-only (root audit is `--omit=dev`); eslint parses our own config, not tenant YAML. |
