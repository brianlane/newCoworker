...
| Clinic sheet patient call | **on** | Webhook, source `clinic_google_sheet` only. Waits 7 minutes (his 5 to 10 minute window), then `place_ai_call` from 09:00 to 18:00. Dane matches Pacific (`America/Los_Angeles`). Any other named clinic uses Eastern (`America/New_York`). A missing clinic name notifies the owner and does not dial. Outside the window the call defers (`outside: defer`), it does not resolve to `not_placed` |
