...
| Tier / box | enterprise, VPS `1806097` (KVM1: 1 vCPU / 4GB, pin stays) |
...
- **Tenant zero for Prospecting.** Our own outbound outreach runs through our
  ...
  2026-09-06: mode `auto`, cap 12, 135 sent). The typed footer postal address
  is waived (Enterprise plan, and also by `HQ_BUSINESS_ID`, so a Standard
  label cannot put the blocker back). HQ was created Standard on 2026-07-16
  by `onboard-hq-tenant.ts`. The dossier called it Enterprise from
  2026-07-26 without a database write. Both `businesses.tier` and
  `subscriptions.tier` were set to enterprise on 2026-09-22. The KVM1 pin
  and central residency (`data_residency_mode = supabase`) were left alone,
  so this did not replace the box. With no address on the business profile,
  the footer is the unsubscribe line alone. That is our own compliance
  judgement. CAN-SPAM still wants a physical address. Since Sep 2026 the first email
