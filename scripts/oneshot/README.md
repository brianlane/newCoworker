...
| `repair-misclaimed-lead-followups.ts` | Reopens the AiFlow follow-up ladders that a mis-routed `"1, <name>"` reply closed (Amy Laidlaw, Aug 2026: a teammate naming a lead that matched none of their live offers had the name read as a claim ETA, which claimed their most-recently-touched offer instead, texted the owner "<teammate> confirmed they spoke with <lead>" about a stranger, and cancelled every AI rung gated on `claimed_agent == "none"`). Matches on the artifact the bug leaves in `actions_taken`, a claim whose ETA does not read as a timeframe by the engine's own `looksLikeTimeframe`, and only for `done` runs carrying a `routing.route_step_id` rewind target. Clears the false claim, resets the `claimed_agent*` vars, rewinds to the route step with a matching resume marker and requeues; leaves `tried`/`offered_log` alone so nobody is re-offered a lead they already passed. Revision-gated so a run the worker touched meanwhile is skipped, not clobbered. Idempotent (a repaired run no longer carries the clause), dry-run by default, ledger-recorded. |
