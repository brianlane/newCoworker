...
| `repair-misclaimed-lead-followups.ts` | Restarts the AI follow-up that a mis-routed `"1, <name>"` reply cancelled (Amy Laidlaw, Aug 2026: a teammate naming a lead that matched none of their live offers had the name read as a claim ETA, which claimed their most-recently-touched offer instead, texted the owner "<teammate> confirmed they spoke with <lead>" about a stranger, and skipped every AI rung gated on `claimed_agent == "none"`). Two fingerprints: a claim whose ETA does not read as a timeframe by the engine's own `looksLikeTimeframe`, or this script's own repair marker next to `routing.owner_assigned` (a repair the owner-assign path closed again). **Clears `contacts.owner_employee_id` before requeuing**, guarded on it still naming the mis-claimer: a claim stamps ownership, the route step hands a lead straight back to an existing owner with no reply, and the first version of this script left that in place, so all four repaired runs re-closed within seconds and re-sent the false confirmation. Resumes at the step AFTER the route step, the AI follow-up itself, because the human answer is already known; `--reask` rewinds onto the route step instead. Revision-gated, idempotent, dry-run by default (reads the ownership predicate rather than writing it), ledger-recorded. |
