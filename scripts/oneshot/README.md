...
| `retire-stale-vps-ssh-keys.ts` | Stamps `rotated_at` on `vps_ssh_keys` rows for boxes we no longer run, so fleet sweeps stop trying to SSH into dead hardware. Nothing rotates a row when a tenant moves to *different* hardware (the partial unique index is per-VPS, so a new box never collides and never triggers a rotation), and the rows accumulate: on 2026-08-14 a chat-worker rollout printed "4/9 succeeded" with five `ssh-failed` timeouts, all five superseded boxes, while all four live tenants updated cleanly. Retires a row only when it is unrotated AND `provider = 'hostinger'` AND not the tenant's current box per `businesses.hostinger_vps_id` AND not `running` per the live Hostinger API, and never the last active row for a business (that would orphan the tenant's break-glass access). Non-destructive: the keypair stays, so reviving a suspended box only needs the stamp cleared. Idempotent, dry-run by default, ledger-recorded. |
