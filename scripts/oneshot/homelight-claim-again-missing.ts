#!/usr/bin/env tsx
/**
 * One-shot: when HomeLight's post-claim page has no "Call me again" button,
 * skip that click and keep the run going.
 *
 * INCIDENT, 2026-09-21, Natasha W., runs 7ce0e9c3 / 06229aed. See
 * homelight-claim-again-missing-definition.ts for the write-up.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/oneshot/homelight-claim-again-missing.ts            # dry run
 *   npx tsx scripts/oneshot/homelight-claim-again-missing.ts --apply
 *   npx tsx scripts/oneshot/homelight-claim-again-missing.ts --revert --apply
 *
...
import {
  ...
  patchDefinition,
  type Definition
} from "./homelight-claim-again-missing-definition";

loadEnv();

const DEFAULT_BUSINESS_ID = "621a5b0d-c2ad-449f-9d74-9d50e7b27fa3"; // Amy Laidlaw Real Estate
const SCRIPT = "homelight-claim-again-missing.ts";
...
async function main(): Promise<void> {
  ...
  if (edits.length === 0) {
    console.log(`\n${CLAIM_AGAIN_ID} missing-control continue already set. Nothing to do (already applied).`);
