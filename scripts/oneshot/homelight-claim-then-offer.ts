#!/usr/bin/env tsx
/**
 * One-shot: claim a HomeLight referral first, then offer the team only when
 * we still have it. Lost leads get an alert, not a press-1 race.
 *
 * INCIDENT, 2026-09-13, Sonia R., run 76248380. See
 * homelight-claim-then-offer-definition.ts for the full write-up. The engine
 * half (persist inbound before trigger eval, skip a second run of the same
 * trigger URL) lives in telnyx-sms-inbound and must be live on main BEFORE
 * this apply, or the withdrawal SMS can still start the run.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/oneshot/homelight-claim-then-offer.ts            # dry run
 *   npx tsx scripts/oneshot/homelight-claim-then-offer.ts --apply
 *   npx tsx scripts/oneshot/homelight-claim-then-offer.ts --revert --apply
 *
...
const SCRIPT = "homelight-claim-then-offer.ts";
...
async function loadInFlight(
  ...
  Array<{ id: string; status: string; resume: string | null }>
> {
  const { data, error } = await db
    .from("ai_flow_runs")
  ...
  return ((data ?? []) as Array<{ id: string; status: string; context: { vars?: Record<string, unknown> } | null }>).map(
    (r) => {
...
async function main(): Promise<void> {
  ...
  if (edits.length === 0) {
    console.log(`\n${OFFER_GATE_ID} already in place. Nothing to do (already applied).`);
