#!/usr/bin/env tsx
/**
 * One-shot: retry a silent HomeLight claim call, text the seller once the
 * contact is in, and stop the portal note from failing unclaimed runs.
 *
 * INCIDENT, 2026-09-14, Sharon I. run 89268fa7 and Brandi V. run 62ecd626.
 * See homelight-nocall-contact-definition.ts for the write-up.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/oneshot/homelight-nocall-contact.ts            # dry run
 *   npx tsx scripts/oneshot/homelight-nocall-contact.ts --apply
 *   npx tsx scripts/oneshot/homelight-nocall-contact.ts --revert --apply
 *
...
const SCRIPT = "homelight-nocall-contact.ts";
...
async function main(): Promise<void> {
  ...
  if (edits.length === 0) {
    console.log(`\n${RECALL_GATE_ID} already in place. Nothing to do (already applied).`);
