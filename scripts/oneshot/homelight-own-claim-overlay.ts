#!/usr/bin/env tsx
/**
 * One-shot: do not tell the team a HomeLight referral went to another agent
 * when WE just claimed it, and do not MMS the post-click overlay.
 *
 * INCIDENT, 2026-09-16, Debra M., run aaeb08fb. See
 * homelight-own-claim-overlay-definition.ts for the write-up.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/oneshot/homelight-own-claim-overlay.ts            # dry run
 *   npx tsx scripts/oneshot/homelight-own-claim-overlay.ts --apply
 *   npx tsx scripts/oneshot/homelight-own-claim-overlay.ts --revert --apply
 *
...
import {
  ...
  patchDefinition,
  type Definition
} from "./homelight-own-claim-overlay-definition";

loadEnv();

const DEFAULT_BUSINESS_ID = "621a5b0d-c2ad-449f-9d74-9d50e7b27fa3"; // Amy Laidlaw Real Estate
const SCRIPT = "homelight-own-claim-overlay.ts";
...
async function main(): Promise<void> {
  ...
  if (edits.length === 0) {
    console.log(`\n${CARD_ID} overlay MMS already cleared. Nothing to do (already applied).`);
