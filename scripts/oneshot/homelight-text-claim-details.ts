#!/usr/bin/env tsx
/**
 * One-shot: treat our HomeLight text claim as ours, and re-read the portal
 * before saying the contact never arrived.
 *
 * INCIDENT, 2026-09-14, Vince N., run e09b3f18. See
 * homelight-text-claim-details-definition.ts for the write-up.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/oneshot/homelight-text-claim-details.ts            # dry run
 *   npx tsx scripts/oneshot/homelight-text-claim-details.ts --apply
 *   npx tsx scripts/oneshot/homelight-text-claim-details.ts --revert --apply
 *
...
const SCRIPT = "homelight-text-claim-details.ts";
...
async function main(): Promise<void> {
  ...
  if (edits.length === 0) {
    console.log(`\n${PORTAL_ID} already in place. Nothing to do (already applied).`);
