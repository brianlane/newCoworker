#!/usr/bin/env tsx
/**
 * One-shot: a HomeLight page that still shows "Call me to claim referral"
 * is not a claim. See homelight-unconfirmed-claim-definition.ts.
 *
 * INCIDENT, 2026-09-23, Ron G., run e7c5e9b3.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/oneshot/homelight-unconfirmed-claim.ts            # dry run
 *   npx tsx scripts/oneshot/homelight-unconfirmed-claim.ts --apply
 *   npx tsx scripts/oneshot/homelight-unconfirmed-claim.ts --revert --apply
 *
...
import {
  ...
  patchDefinition,
  type Definition
} from "./homelight-unconfirmed-claim-definition";

loadEnv();

const DEFAULT_BUSINESS_ID = "621a5b0d-c2ad-449f-9d74-9d50e7b27fa3"; // Amy Laidlaw Real Estate
const SCRIPT = "homelight-unconfirmed-claim.ts";
...
async function main(): Promise<void> {
  ...
  if (edits.length === 0) {
    console.log(`\nUnconfirmed-claim rule already set. Nothing to do (already applied).`);
