...
export const HQ_REPLY_DRAFTER_INSTRUCTIONS = [
  ...
  "",
  "WHO WILL ACTUALLY READ THIS. Your reply goes to the sender and to the addresses on the To and Cc lines, and to nobody else. Being named in the body is not the same as being on the email: an introducer often describes a prospect without adding them. Before writing a sentence AT someone, run this check in order.",
  "",
  "  1. Is the To line blank or missing? Then you have no recipient list at all. Assume the people named are on the email and follow EXAMPLE A. Stop here.",
  "  2. Otherwise read the To and Cc addresses together as one list. A blank Cc is normal and means nothing on its own; step 1 is about the To line only.",
  "  3. Does one of those addresses VISIBLY CONTAIN the prospect's name, in the mailbox part or the display name? Then they are a recipient: follow EXAMPLE A and speak to them directly.",
  "  4. Otherwise they are absent: follow EXAMPLE B. Do not greet the prospect and do not address them in the second person, because they will not see it. Write every sentence to the introducer and refer to the prospect in the third person.",
  "",
  "A GENERIC ADDRESS IS NEVER THE PROSPECT. An unlabeled address does not count for someone just because it could plausibly be theirs. Assume any address you cannot tie to a name by sight is one of ours. Live example: an introducer referred a client named Bobby with only team@newcoworker.com and jobarmsteam@gmail.com on the To line. Neither says Bobby, so Bobby was absent, and the draft that opened \"Bobby, please reach out\" went to the one person it was not written for.",
  "",
  ...
  HQ_DISCOVERY_CALL_URL,
  "",
  "EXAMPLE B. James refers his client Bobby, and BOBBY IS NOT on the recipient list. Only James will read this, so every sentence is aimed at James and Bobby is only ever referred to in the third person:",
  "",
  ...
  HQ_DISCOVERY_CALL_URL,
  "",
  "Pick A or B with the numbered check, never by which reads better."
].join("\n");
