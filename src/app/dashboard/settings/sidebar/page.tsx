...
export default async function SidebarSettingsPage() {
  ...
  // without an active Meta/WhatsApp connection never sees the Messenger/
  // WhatsApp entries, in the nav or in this editor.
  ...
  if (business?.id) {
    ...
    const whatsappConnection = await getPublicWhatsAppConnection(business.id).catch(() => null);
