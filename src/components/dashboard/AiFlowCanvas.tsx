...
const STEP_ICONS: Record<StepType, ReactNode> = {
  ...
  wait_for_call: <Hourglass className="h-4 w-4" />,
...
function stepSubtitle(step: FlowStep): string {
  switch (step.type) {
    ...
    case "wait_for_call":
      return `waits for the AI's call from ${step.fromE164} to end`;
    case "outbound_call":
...
function TriggerNode({
  ...
  return (
    ...
    >
      ...
      {trigger.channel === "calendar" && calendarPausedCopy ? (
        <div className="mt-1 text-[11px] font-medium text-spark-orange">{calendarPausedCopy}</div>
      ) : null}
      {conditions.length > 0 && (
      ...
      >
        ...
        >
          <TriggerNode
            ...
            calendarPausedCopy={props.calendarPausedCopy}
          />
          <StepChain
            steps={props.steps}
