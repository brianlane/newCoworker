    ...
    <>
      ...
      >
        ...
        {viewAsActive ? (
          ...
          <CompanionChatBody businessId={businessId} />
        )}
      </div>
    </>
  );
...
function CompanionChatBody({ businessId }: { businessId: string }) {
  const t = useTranslations("dashboard.companion");
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="border-t border-parchment/10 bg-deep-ink/40 p-3">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <button
            type="submit"
            disabled={sending || isPaused || !input.trim() || input.trim().length > MAX_MESSAGE_CHARS}
