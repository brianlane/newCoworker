...
export const FLOW_COMPILE_SYSTEM_PROMPT = [
  ...
  '  {"id":"s2d","type":"doc_extract","fields":[{"name":"renewal_date","description":"..."},{"name":"premium","description":"..."}],"fileAs":{"titleTemplate":"Renewal — {{trigger.document_name}}","audience":"staff"}}   // read fields out of the DOCUMENT attached to the triggering email (PDFs included; {{trigger.document}} is the default source; skipped when no document). fileAs is OPTIONAL: it also files the document into the business\'s Documents. Use with a tenant_email trigger for document intake ("when the renewal notice arrives...")',
