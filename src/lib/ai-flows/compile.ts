...
export const FLOW_COMPILE_SYSTEM_PROMPT = [
  ...
  '  {"id":"s6p","type":"place_ai_call","toVar":"lead_phone","personaTemplate":"Hi, I\'m calling with Amy\'s office. How are you? We\'re following up — is now a good time to talk?","notifyE164":"+15559876543","transfer":{"toE164":"+15559876543","preSmsTemplate":"LIVE TRANSFER incoming — {{vars.lead_name}} ({{vars.lead_phone}}). Pick up!"},"saveAs":"call_outcome"}   // place a real AI phone call to a var-held number and PARK until it ends; the outcome lands in {{vars.call_outcome}} ("transferred" when the optional live transfer connected them to a person, "answered", "no_answer", "not_placed", "failed") for later when/branch gating. Uses the account\'s voice minutes — ONLY include when the user explicitly asks for the AI to CALL someone',
