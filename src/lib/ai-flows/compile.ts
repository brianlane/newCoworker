...
export const FLOW_COMPILE_SYSTEM_PROMPT = [
  ...
  "wait_for_reply is parked, the lead's next text is captured into the saveAs",
  "var so later when/branch steps can skip remaining nudges. The default SMS",
  "coworker still replies UNLESS options.suppressDefaultReply is true: set that",
  "when the flow itself will send the next customer text (an ack, a",
  "classify-then-reply). Leave it off for a silence-nudge cadence, so a lead",
  "who texts back is handed to the coworker. Use sleep",
  ...
  '{"var":"x","notEquals":"v"} / {"var":"x","blank":true}. blank matches a missing',
  'or whitespace-only value. extract_text writes "" when a field is absent, and',
  'that "" is not equal to "none", so use blank for a missing field and a',
  'separate equals "none" arm when the sentinel should take the same path. Pair',
  'equals/notEquals on the same var for an',
  ...
  "run, continueWhenText does not). A browse_extract may also carry",
  '"forceWhenText":[{"contains":"Call me to claim referral","set":{"claim_state":"NOT CONFIRMED, claim by hand now"}}]',
  "so that when the page still shows that phrase, those vars overwrite whatever",
  "the model extracted. Use it when a click can report success while the",
  "original button is still on the page: the page wins, the model does not.",
  "A browse_action may also carry",
  '"continueWhenMissingControl":true to skip only THAT STEP when click_text',
  'finds no matching control after waiting (the button is gone, not late; pair',
  'with "missingControlSaveAs":"claim_again_click" to write "missing" so a later',
  "when can skip the wait or text the owner), and \"auth\":{\"integrationLabel\":",
