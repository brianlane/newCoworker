...
export const FLOW_COMPILE_SYSTEM_PROMPT = [
  ...
  '  {"id":"s6p","type":"place_ai_call","toVar":"lead_phone","personaTemplate":"Hi, I\'m calling with Amy\'s office. How are you? We\'re following up — is now a good time to talk?","contextTemplate":"Their name: {{vars.lead_name}}. Address: {{vars.lead_address}}.","notifyE164":"+15559876543","transfer":{"toE164":"+15559876543","preSmsTemplate":"LIVE TRANSFER incoming — {{vars.lead_name}} ({{vars.lead_phone}}). Pick up!"},"saveAs":"call_outcome"}   // place a real AI phone call to a var-held number and PARK until it ends; the outcome lands in {{vars.call_outcome}} ("transferred" when the optional live transfer connected them to a person, "answered", "no_answer", "not_placed", "failed") for later when/branch gating. Uses the account\'s voice minutes — ONLY include when the user explicitly asks for the AI to CALL someone',
  ...
  '  {"id":"s7","type":"route_to_team","offerTemplate":"New lead {{vars.lead_name}}, reply 1 to claim or 2 to pass by {{offer.deadline}}","responseMinutes":10,"ownerFallbackTemplate":"No agent claimed {{vars.lead_name}}","claimedNotifyTemplate":"{{agent.name}} claimed {{vars.lead_name}}","agentName":"<optional: pin offers to this roster member>","offerWindow":{"timezone":"America/Phoenix","quietStart":"21:00","quietEnd":"08:30","graceMinutes":10},"attachScreenshot":true}   // add "agentNames":["<name>","<name>"] INSTEAD of agentName to offer 2-10 roster members simultaneously (one shared deadline, first "1" wins, "2" retires just that person, everyone passing / the deadline lapsing falls back to the owner) — use it when the user says a lead should go to several people at once; or add "broadcastAll":true INSTEAD of any names to offer EVERYONE on the active roster at once (resolved at run time, capped at 10, same claim semantics) — use it when the user says the whole team should be offered',
  ...
  '"offer everyone on the team". An optional offerWindow',
