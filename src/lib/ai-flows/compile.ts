...
export const FLOW_COMPILE_SYSTEM_PROMPT = [
  ...
  '  {"id":"s3b","type":"send_email","to":"owner@example.com","cc":["manager@example.com"],"bcc":["archive@example.com"],"subject":"{{vars.lead_name}} lead","body":"...","attachScreenshot":true}   // add "attachDocumentTemplate":"business-docs:{{vars.<saveAs>_document_id}}" to attach a business document to the email: typically the document a preceding run_agent step filed via saveDocument (its id lands in that var), or a fixed "business-docs:<document uuid>". AI-coworker email only (not combinable with fromConnectionId); a ref that renders empty sends without the attachment. Add "replyToEmailLogId":"{{trigger.email_log_id}}" to send it as a REPLY on the triggering conversation instead of opening a new one: the reply lands inside the original thread in the recipient\'s mailbox, and the coworker then owns that thread for later messages. Email triggers only; a blank render sends unthreaded',
  '  {"id":"s3o","type":"email_organize","messageIdTemplate":"{{trigger.message_id}}","connectionId":"<uuid of a connected mailbox OR omit for the AI mailbox>","addLabels":["Sales"],"moveToFolder":"Sales","archive":true,"markRead":true,"markUnread":false,"unarchive":false,"trash":false,"removeLabels":[]}   // organize the triggering email: apply labels, move to a folder, archive, mark read/unread, and/or move to trash. "trash":true bins it in Gmail/Outlook (recoverable for 30 days, never a permanent delete) and runs after any labelling, so the message stays findable by label in the bin; do not combine it with unarchive or markUnread. For a CONNECTED Gmail/Outlook mailbox set connectionId to a real AVAILABLE MAILBOXES uuid; for the AI coworker mailbox OMIT connectionId. Pair with classify → branch for inbox triage. At least one of archive/unarchive/trash/markRead/markUnread/addLabels/removeLabels/moveToFolder is required',
  '  {"id":"s4","type":"approval_gate","prompt":"..."}   // texts the owner numbered options and parks until they answer. Add "allowModify":{"redraftStepId":"<id of an earlier step>"} to also accept a free-text answer ("shorter, drop the second paragraph"): the run rewinds to that step with what they said and asks again. Use it whenever the thing being approved is DRAFTED text, since a digit cannot express a change',
