...
export async function classifyOwnerAsk(args: {
  ...
  generate?: typeof geminiGenerateText;
  ...
  const generate = args.generate ?? geminiGenerateText;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), args.timeoutMs ?? ASK_CLASSIFIER_TIMEOUT_MS);
  try {
    const raw = await generate({
      apiKey: args.apiKey,
      model: args.model ?? ASK_CLASSIFIER_MODEL,
      systemInstruction: OWNER_ASK_CLASSIFIER_PROMPT,
      userText: message.slice(0, ASK_CLASSIFIER_MAX_INPUT_CHARS),
      temperature: 0,
      maxOutputTokens: 200,
    ...
    return parseAskClassification(raw);
  } catch {
    return UNKNOWN_ASK;
  } finally {
    clearTimeout(timer);
