 ...
 *
 * Markup mirrors the Supabase Auth-style templates (dark shell, green CTA).
...
/** Escape a URL for use in an HTML double-quoted attribute. */
...
export type BrandedEmailHtmlInput = {
  ...
  /** When true (default), render fallback URL section using `cta.href` or `fallbackHref`. */
  includeFallbackLink?: boolean;
  fallbackHref?: string;
  warningLine?: string;
  securityNote?: string;
  recipientEmail: string;
  /** One-click unsubscribe URL (urgent alerts / digest). Shown as muted footer line. */
...
export function buildBrandedEmailHtml(input: BrandedEmailHtmlInput): string {
  ...
  const signatureBlock = `<tr><td style="padding:0 40px 32px;">
