...
const TAGS: Record<OrphanSweepFinding["kind"], string> = {
  pooled: "[POOLED]",
  would_pool: "[WOULD POOL]",
...
function findingLine(finding: OrphanSweepFinding): string {
  ...
  return (
    ...
    `${created}${expires}): ${finding.detail} ${TAGS[finding.kind]}`
  );
}
...
export function buildOpsOrphanSweepEmail(input: OpsOrphanSweepInput): OpsOrphanSweepEmail {
  ...
  const wouldPool = input.findings.filter((f) => f.kind === "would_pool").length;
  // Only genuinely-stuck boxes escalate. Counting a dry run's poolable boxes
  // here made a clean dry run shout ACTION REQUIRED about boxes the live
  // sweep would have handled on its own.
  const actionCount = input.findings.filter((f) => f.kind === "reported").length;
  const prefix = input.dryRun ? "[ops][dry run]" : "[ops]";
  const handled = input.dryRun ? wouldPool : pooled;
  const handledPhrase = input.dryRun
    ? `${handled} untracked paid box(es) would be pooled for reuse`
    : `${handled} untracked paid box(es) pooled for reuse`;
  const subject =
    actionCount > 0
      ? `${prefix} ACTION REQUIRED: ${actionCount} untracked Hostinger VM(s) the sweep would not touch`
      : `${prefix} Orphan sweep: ${handledPhrase}`;
  ...
  const textLines = [
    ...
    input.dryRun
      ? "WOULD POOL boxes were left untouched by this dry run. A live run would turn auto-renew off and pool them as adopt-first inventory, so a signup of the matching size could reuse one and any that went unadopted would lapse at its paid period end. Nothing here needs a human."
