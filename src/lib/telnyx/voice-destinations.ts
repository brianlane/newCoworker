/**
 * The canonical Telnyx destination allowlist, shared by BOTH writers of
 * whitelisted_destinations: the widen one-shot
 * (scripts/oneshot/widen-telnyx-destinations.ts, which PATCHes existing
 * profiles as a monotone union) and per-tenant profile CREATION
 * (tenant-voice-infra.ts). One source, so a tenant profile created today can
 * never reach fewer countries than the shared profile it replaces.
 *
 * Moved here from scripts/oneshot/widen-telnyx-allowlist.ts (which now
 * re-exports) so src/lib provisioning code can depend on it without
 ...
 * its own whitelist region. On Aug 5 2026 a widen run replaced the
 ...
 * the profiles were re-patched on Aug 6. Hence two defenses: regions Telnyx
 * knows but the dial table cannot express are added explicitly, and
 * `assertContainsLiveTrafficRegions` refuses to produce a list that lacks a
 * region the fleet actually texts or calls today.
 */
import {
  SMS_DIAL_CODES,
