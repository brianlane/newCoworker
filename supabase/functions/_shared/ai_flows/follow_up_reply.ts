 ...
 * real; it simply had no contact row yet. So this text now says three things
 * instead of one: what was actually searched (contacts on file), why a lead we
 * clearly know about can still be missing from it (the referral site has not
 * released their details), and what to do about it (re-send "F" once they show
 * on the dashboard).
 *
 * That last clause is the entire remedy for the withheld-details window, and
 * it is deliberately all of it. PR #1702 also built a mechanism that parked
 * the request on the live run and applied it automatically when the contact
 * landed. It worked, but it needed six rounds of correction, and every defect
 * it produced was a way for a teammate to be told something untrue: stand down
 * on an unclaimed lead, reply "1" to an offer that does not exist, or nothing
 * at all after being promised a text. One honest sentence carries almost all
 * of the value and cannot mislead anyone (PR #1705).
 */
