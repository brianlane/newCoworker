/**
 * Destination-country resolution and cost multipliers (Deno lockstep copy).
 *
 * Mirror of `src/lib/sms/destination-rates.ts`; edge functions cannot import
 * src/, and the worker-integration fixture matrix keeps this copy, the src
 * copy, and the SQL `sms_dial_codes` table in agreement. See the src file
 * for the full rationale (rate deck, per-part caveat, MX exclusion).
 */
