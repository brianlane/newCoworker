/**
 * Supabase Edge Function: abandoned-signup-sweep
 *
 * Cron bridge for the daily abandoned-signup cleanup. pg_cron hits this with
 * `Authorization: Bearer <INTERNAL_CRON_SECRET>`; we validate the bearer, then
 * forward to the Next.js internal endpoint which holds the sweep logic (the
 * Supabase service client and the cascade delete need the Node runtime), same
 * indirection as monthly-intro-nudge-sweep.
 *
 * Environment:
...
const TARGET_PATH = "/api/internal/abandoned-signup-sweep";
  ...
  try {
    const res = await fetch(target, {
      ...
      headers: {
        ...
        "X-Cron-Job": "abandoned-signup-sweep",
    ...
    console.error("abandoned-signup-sweep bridge failure", { target, message });
