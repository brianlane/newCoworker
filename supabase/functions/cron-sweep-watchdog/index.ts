/**
 * Supabase Edge Function: cron-sweep-watchdog
 *
 * Cron bridge for the daily sweep-fleet health check. pg_cron hits this at
 * 03:30 UTC with `Authorization: Bearer <INTERNAL_CRON_SECRET>`; we validate
 * the bearer and forward to the Next.js internal endpoint that holds the
 * logic (it needs the service-role client and the Resend sender, both Node
...
const TARGET_PATH = "/api/internal/cron-sweep-watchdog";
// Two indexed reads and one email. Set to the platform ceiling rather than
// the 290_000 most bridges carry, since anything above 150s is unreachable
// anyway and the matching route declares maxDuration = 150.
  ...
  try {
    const res = await fetch(target, {
      ...
      headers: {
        ...
        "X-Cron-Job": "cron-sweep-watchdog",
    ...
    console.error("cron-sweep-watchdog bridge failure", { target, message });
