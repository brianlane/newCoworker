/**
 * Supabase Edge Function: email-campaign-sweep
 *
 * Cron bridge for the per-minute email-campaign sender. pg_cron hits this
 ...
 * sending logic (Resend + templates need the Node runtime), same
 * indirection as document-expiration-sweep.
 *
 * Environment:
...
const TARGET_PATH = "/api/internal/email-campaign-sweep";
  ...
  try {
    const res = await fetch(target, {
      ...
      headers: {
        ...
        "X-Cron-Job": "email-campaign-sweep",
    ...
    console.error("email-campaign-sweep bridge failure", { target, message });
