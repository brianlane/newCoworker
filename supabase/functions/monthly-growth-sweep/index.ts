/**
 * Supabase Edge Function: monthly-growth-sweep
 *
 * Cron bridge for the daily monthly growth-recap sweep.
 ...
 * which holds the sweep logic (Resend + the message catalog need the Node
 * runtime), same indirection as monthly-intro-nudge-sweep.
 *
 * The sweep runs daily and sends nothing before the 3rd of the month; the
 * daily tick is what gives a failed send a retry inside the same month.
 *
 * Environment:
...
const TARGET_PATH = "/api/internal/monthly-growth-sweep";
  ...
  try {
    const res = await fetch(target, {
      ...
      headers: {
        ...
        "X-Cron-Job": "monthly-growth-sweep",
    ...
    console.error("monthly-growth-sweep bridge failure", { target, message });
