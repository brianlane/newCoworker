/**
 * Supabase Edge Function: segment-action-sweep
 *
 * Cron bridge for the nightly Smart List action sweep. pg_cron hits this
 * with `Authorization: Bearer <INTERNAL_CRON_SECRET>`; we validate the
 * bearer, then forward to the Next.js internal endpoint which holds the
 * sweep logic (supabase/functions/_shared/segment_actions.ts, run under the
 * Node runtime), same indirection as email-campaign-sweep and
...
const TARGET_PATH = "/api/internal/segment-action-sweep";
  ...
  try {
    const res = await fetch(target, {
      ...
      headers: {
        ...
        "X-Cron-Job": "segment-action-sweep",
    ...
    console.error("segment-action-sweep bridge failure", { target, message });
