/**
 * Supabase Edge Function: tendlc-attach-retry
 *
 * Cron bridge for the 10DLC (A2P SMS) campaign-attach retry worker.
 *
 * pg_cron hits this Edge function every minute with
 *   Authorization: Bearer <INTERNAL_CRON_SECRET>.
 * We validate the bearer, then forward to the Next.js internal endpoint
 * which contains the actual Telnyx + DB logic, same indirection as the
 * grace-sweep bridge so all 10DLC business rules live in one Node runtime.
 *
 * Environment:
...
const TARGET_PATH = "/api/internal/tendlc-attach-retry";
  ...
  try {
    const res = await fetch(target, {
      ...
      headers: {
        ...
        "X-Cron-Job": "tendlc-attach-retry",
    ...
    console.error("tendlc-attach-retry bridge failure", { target, message });
