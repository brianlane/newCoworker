/**
 * Supabase Edge Function: vps-orphan-sweep
 *
 * Cron bridge for the daily fleet orphan sweep.
 *
 * pg_cron hits this Edge function daily with
 *   Authorization: Bearer <INTERNAL_CRON_SECRET>.
 * We validate the bearer, then forward to the Next.js internal endpoint
 * which contains the actual Hostinger + DB logic.
 *
 * Environment:
...
const TARGET_PATH = "/api/internal/vps-orphan-sweep";
// Matches the route's maxDuration (300s): the sweep is a handful of Hostinger
// reads plus one detail lookup per orphan, not a provisioning run.
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.error("vps-orphan-sweep bridge failure", { target, message });
