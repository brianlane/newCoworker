-- Never-answered hangup used to leave a voice_settlements row that could
-- never finalize. telnyx-voice-inbound releases the reservation on Telnyx
-- 90018 ("Call has already ended"), then telnyx-voice-call-end still upserted
-- a settlement. voice_try_finalize_settlement refused reservation_released,
-- the 15-minute sweep kept returning 0, and voice-bridge-health-alerts paged
-- hourly after 30 minutes. 2026-09-09 Amy Laidlaw: v3:OtTLp2osf-…, 1s ring,
-- no answer, no websocket, minutes already released.
--
-- Two halves:
--   1. Finalize a released + never-connected reservation at 0 billable
--      (self-heal leftovers the hangup path used to write).
--   2. One-shot UPDATE for any rows already sitting open. Idempotent.
...
-- Close leftovers the old hangup path already wrote. Minutes were returned
-- when the reservation released; this only stamps the settlement so the
-- health cron stops paging.
update public.voice_settlements s
set
  billable_seconds = 0,
  finalized_at = now(),
  no_turns_zero_billed = true,
  settlement_idempotency_key = coalesce(s.settlement_idempotency_key, gen_random_uuid()::text)
