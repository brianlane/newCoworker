 ...
 * CLOSED), skipped for forward_owner contacts (the owner already receives
 * those texts verbatim), deduped per JOB across retry claims (a retry whose
 * first claim failed to page still alerts), and coalesced per contact so a
 * multi-part text pages once.
 */

const BIZ = "00000000-0000-0000-0000-000000000001";
const LEAD = "+17808039935";
const JOB = "30000000-0000-0000-0000-000000000001";
...
describe("sendCustomerReplyAlert", () => {
  it("pages the owner with the contact- and job-stamped payload", async () => {
    const fetchFn = okFetch();
    const { db } = makeDb([OPTED_IN, NAMED_CONTACT, NO_PRIOR, NO_PRIOR]);
    ...
    expect(body.record.log_payload.job_id).toBe(JOB);
  ...
  it("labels by number when no contact row exists, and tolerates a contact read error", async () => {
    const fetchFn = okFetch();
    const { db } = makeDb([OPTED_IN, { data: null, error: null }, NO_PRIOR, NO_PRIOR]);
  ...
  it("a DELIVERED page for this exact job never re-pages (retry claim)", async () => {
    const fetchFn = okFetch();
    const { db } = makeDb([OPTED_IN, NAMED_CONTACT, { data: [{ id: "n1" }], error: null }]);
    const result = await sendCustomerReplyAlert(db, baseInput(fetchFn));
  ...
  it("null dedupe/coalesce results (no rows, no error) alert", async () => {
    const fetchFn = okFetch();
    const { db } = makeDb([
      OPTED_IN,
      NAMED_CONTACT,
