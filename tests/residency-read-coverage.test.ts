...
const PURGED_READ_CENTRAL_BY_DESIGN: Record<SiteKey, string> = {
  ...
  "src/lib/db/notifications.ts::notificationBusinessId::notifications":
    "asked by the push click receipt ONLY to disambiguate which scope a tap belonged to when one browser holds subscriptions under two of them (an HQ admin who is also an owner, same endpoint). The overwhelmingly common single-scope tap never reaches this read at all. For a vps tenant whose row central has purged it returns null, and the receipt then records NOTHING rather than guessing, which is the fail-safe direction: a fabricated liveness signal is the exact failure channel-liveness exists to prevent, and that sweep already refuses to judge a vps tenant (residencySkipReason) so the missing receipt is not read by anything. Routing box-ward is not an option here either, because the whole point of the read is to DISCOVER which business owns the notification, so there is no tenant in hand to pick a box with",
  "src/lib/db/notifications.ts::markWhatsAppAlertUndelivered::notifications":
    "the read half of a read-modify-write whose write is CENTRAL, on a row this system wrote centrally seconds earlier. A WhatsApp delivery receipt lands about fifteen seconds after the send, and this pair exists to walk the failure back to the alert row the dispatcher had already recorded as `sent`. Two reasons central is the only correct side. The row is far inside the 72h purge keep floor by construction, so central always has it: a fifteen-second-old notification is never a purge candidate. And routing the read alone would be actively wrong rather than merely slow, exactly as with the scheduled_sms pair below: on a vps tenant it would find the box copy of a row whose central twin the UPDATE then fails to match, and the correction would be silently lost while reporting success. The journal triggers mirror the central UPDATE box-ward anyway, so the box ends up with the corrected status either way",
  "src/lib/notifications/channel-liveness-read.ts::countSendsByChannel::notifications":
    "the sweep REFUSES to judge a vps tenant at all: residencySkipReason() drops it before any read, and the skip is reported in the result so the silence cannot read as a clean bill of health. That refusal is the correct answer here rather than routing, because the purge predicate deletes READ notifications specifically, so a central read would return a set thinned in exactly the direction this check measures: fewer sends AND the read_at signal preferentially destroyed, which reads as 'quiet and unread' and would report a healthy tenant as dark. A wrong alarm about who is reachable is worse than no alarm",
  "src/lib/notifications/channel-liveness-read.ts::lastDashboardReadAt::notifications":
