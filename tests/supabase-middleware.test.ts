...
describe("proxy", () => {
  ...
  it("allows POST to /api/internal without origin (bearer-authed worker/cron callback)", async () => {
    mockSupabaseWithUser(null);
    const req = makeRequest("/api/internal/dashboard-chat-summarize", { method: "POST" });
    const res = await proxy(req);
    expect(res.status).not.toBe(403);
  });
