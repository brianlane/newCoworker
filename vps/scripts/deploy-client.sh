...
if [[ -f "${CHAT_WORKER_DEST}/docker-compose.yml" ]]; then
  ...
  (
    ...
    # NEXT_PUBLIC_APP_URL is the canonical source for the Vercel base
    # URL (the name used throughout this codebase — not Vercel's
    # auto-injected NEXT_PUBLIC_SITE_URL). INTERNAL_CRON_SECRET is
    # the same secret asserted by /api/internal/* routes via
    # assertCronAuth in src/lib/cron-auth.ts.
...
ROWBOAT_GATEWAY_TOKEN=${ROWBOAT_GATEWAY_TOKEN}
BUSINESS_ID=${BUSINESS_ID}
WORKER_VERCEL_BASE_URL=${NEXT_PUBLIC_APP_URL:-${WORKER_VERCEL_BASE_URL:-}}
