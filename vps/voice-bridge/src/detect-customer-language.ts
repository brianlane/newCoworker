/**
 * Voice-bridge-local copy of shared/i18n/detect-customer-language.ts (the
 * bridge is rsynced to each tenant VPS standalone, so it cannot import across
 * the repo). Same arrangement as customer-language-line.ts and voice-name.ts.
 *
 * The RETURNED RESULT must stay identical to the shared module for the same
 * inputs; tests/voice-bridge-detect-language-lockstep.test.ts pins the two
 * against a corpus so a one-sided edit is loud.
 *
 * Why voice needs it at all: the translator gate (translator-gate.ts) has to
 * decide, at the moment of a warm transfer, whether the two people about to be
 * bridged actually speak different languages. Before this, that decision read
 * only the contact's stored preference, so an unknown caller produced no
 * language at all and the interpreter cue was built with none (call 5634b7f0,
 * 2026-08-18). Reusing the detector the SMS side already learns language with
 * keeps one definition of "this person is speaking Spanish" across channels.
 */
...
export type DetectCustomerLanguageOpts = {
  ...
  establishedLanguage?: VoiceCustomerLanguage | null;
  defaultLanguage?: VoiceCustomerLanguage;
  supported?: VoiceCustomerLanguage[];
};

export type DetectCustomerLanguageResult = {
  language: VoiceCustomerLanguage;
